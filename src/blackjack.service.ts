import type { Card, DealerHand, Game, Hand } from './types.js';
...
export function initializeRound(game: Game, bet: number): Game {
  ...
  if (handValue(playerHand.cards) === BLACKJACK_VALUE)
    playerHand.status = HandStatus.Blackjack;
  if (handValue(dealerHand.cards) === BLACKJACK_VALUE)
...
export function resolveHandStatus(
  cards: Card[],
):
  | typeof HandStatus.Playing
  | typeof HandStatus.Stood
  | typeof HandStatus.Busted {
...
export function dealerPlay(game: Game) {
  ...
  if (game.dealer.status !== HandStatus.Blackjack) {
    game.dealer.status =
      handValue(game.dealer.cards) > BLACKJACK_VALUE
        ? HandStatus.Busted
        : HandStatus.Stood;
  }
}
...
function resolveBets(game: Game): void {
  ...
  for (const hand of game.player) {
    ...
    if (hand.status === HandStatus.Busted) {
      ...
      dealer.status === HandStatus.Busted ||
      handValue(hand.cards) > handValue(dealer.cards)
    ) {
      hand.outcome = HandOutcome.Win;
...
export function getBetValidationError(balance: number, bet: number) {
  if (!Number.isFinite(bet) || bet <= 0)
    return { status: 400, message: 'Bet must be a positive number' };
  if (bet > balance)
    return { status: 409, message: 'Insufficient balance for this bet' };
  return undefined;
}
