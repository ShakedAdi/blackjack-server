...
export function createGame(
  req: Request<unknown, unknown, { bet: number }>,
  res: Response,
): void {
  const { bet } = req.body;
  ...
  res
    ...
    .json({
      gameId,
      dealersCard: first(game.dealer.cards),
      playersHand: first(game.player),
      balance: game.balance,
    });
}

export function newRound(
  req: Request<{ gameId: string }, unknown, { bet: number }>,
  res: Response,
): void {
...
export function split(req: Request<{ gameId: string }>, res: Response): void {
  ...
  const hand = game.player.find(
    (h) =>
      h.status === HandStatus.Playing &&
      h.cards.length === INITIAL_HAND_SIZE &&
  ...
  advanceGameState(game);
  const firstHandCards = hand.cards.map((card) => ({
    rank: card.rank,
    suit: card.suit,
  }));
  const secondHandCards = newHand.cards.map((card) => ({
    rank: card.rank,
    suit: card.suit,
  }));
  res
    ...
    .json({
      firstHandCards,
      firstHandStatus: hand.status,
      secondHandCards,
      secondHandStatus: newHand.status,
    });
}
...
export function double(req: Request<{ gameId: string }>, res: Response): void {
  ...
  const hand = game.player.find(
    (h) =>
      h.status === HandStatus.Playing && h.cards.length === INITIAL_HAND_SIZE,
  );
  if (!hand) {
  ...
  hand.cards.push(newCard);
  hand.status =
    handValue(hand.cards) > BLACKJACK_VALUE
      ? HandStatus.Busted
      : HandStatus.Stood;
...
export function getGameState(
  ...
  const dealer: Card[] = game.dealer.isHoleCardHidden
    ? [first(game.dealer.cards)]
    : game.dealer.cards;
  res
    .status(200)
    .json({
      state: game.state,
      isHoleCardHidden: game.dealer.isHoleCardHidden,
      player: game.player,
      dealer,
      balance: game.balance,
    });
